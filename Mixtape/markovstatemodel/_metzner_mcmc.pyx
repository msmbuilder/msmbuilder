...
def metzner_mcmc_fast(Z, int n_samples, int n_thin=1, random_state=None):
    ...
    for i in range(n_samples / n_thin):
        rand = random.rand(4 * n_thin)
        metzner_mcmc_step(
            &Zcopy[0,0], &N[0], &K[0,0], &Q[0], &rand[0],
            &sc, n_states, n_thin);

        yield np.array(K) / np.array(Q)[:, np.newaxis]
