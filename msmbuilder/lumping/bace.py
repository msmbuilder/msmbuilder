...
class BACE(MarkovStateModel):
    ...
    def getInds(self, c, stateInds, updateSingleState=None):
        ...
        for s in stateInds:
            ...
            if dest.shape[0] == 0:
                continue
            elif dest.shape[0] < self.chunkSize:
                indices.append((s, dest))
            else:
                i = 0
                while dest.shape[0] > i:
                    if i+self.chunkSize > dest.shape[0]:
                        indices.append((s, dest[i:]))
                    else:
                        indices.append((s, dest[i:i+self.chunkSize]))
                    i += self.chunkSize
        return indices
    ...
    def multiDist(self, indicesList, c, w, statesKeep, unmerged):
        d = np.zeros((len(indicesList), self.chunkSize), dtype=np.float32)
    ...
    def filterFunc(self, c):
        ...
        if nInd > 1 and self.n_proc > 1:
            ...
            d = self.multiDistHelper(indices, pseud, 1, c, w, statesKeep, unmerged)
